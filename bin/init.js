...
`,
  ...
  const configFile = existsSync(pkg("next.config.mjs"))
    ? "next.config.mjs"
    : existsSync(pkg("next.config.js"))
      ? "next.config.js"
      : null;
...
`,
  ...
  if (layout) {
    appendIfMissing(
      ...
      `import virtual:uno.css → ${layout}`,
    );
  } else {
    skip(
      "layout.jsx/tsx no encontrado — añade import 'virtual:uno.css' manualmente",
    );
  }
}
  ...
  if (existsSync(pkg(configFile))) {
    ...
    if (content.includes("UnoCSS")) {
      ...
      const patched =
        ...
        content.replace(/integrations:\s*\[/, `integrations: [\n    UnoCSS(),`);
      writeFileSync(pkg(configFile), patched);
      log(`${configFile} — UnoCSS añadido`);
    }
  } else {
